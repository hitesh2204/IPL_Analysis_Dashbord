...
def team_vs_team_analysis(ipl, team1, team2):
    try:
        ...
        # Filter matches between the teams
        ...
        ax.pie(win_counts, labels=win_counts.index, autopct='%1.1f%%', startangle=50,
        ...
        logger.info(f"✅ Team vs Team Analysis completed for {team1} vs {team2} with {total_matches} matches.")
    
    except Exception as e:
        logger.exception(f"❌ Error during team_vs_team_analysis({team1}, {team2}): {e}")
        st.error("Something went wrong while analyzing the team vs team data. Please check logs for more details.")
